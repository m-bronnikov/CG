...
GLfloat* get_figure(){
    GLfloat* vertex = (GLfloat*) malloc(2 * sizeof(GLfloat) * STEPS * 3 * STEPS * 2 * 3 * 3);
    std::cout <<  2 * STEPS * 3 * STEPS * 2 * 3 * 3 << std::endl;
    int j = 0;
    // идем по окружности
    for(int i = 0; i < STEPS; ++i){
        // отрисовываем бок
        for(int k = 0; k < STEPS; ++k){
            GLfloat ax = cos(2.0 * M_PI * (float)i / STEPS) - cos(2.0 * M_PI * (float)(i + 1) / STEPS);
            GLfloat ay = 0.0f;
            GLfloat az = sin(2.0 * M_PI * (float)i / STEPS) - sin(2.0 * M_PI * (float)(i + 1) / STEPS);
            ...
            //first triangle
            vertex[j++] = 0.6 * cos(2.0 * M_PI * (float)i / STEPS);
            vertex[j++] = (float)k / STEPS * chill_height;
            vertex[j++] = 0.6 * sin(2.0 * M_PI * (float)i / STEPS);
            ...
            vertex[j++] = 0.6 * cos(2.0 * M_PI * (float)(i + 1) / STEPS);
            vertex[j++] = (float)k / STEPS * chill_height;
            vertex[j++] = 0.6 * sin(2.0 * M_PI * (float)(i + 1) / STEPS);
            ...
            vertex[j++] = 0.6 * cos(2.0 * M_PI * (float)i / STEPS);
            vertex[j++] = (float)(k + 1) / STEPS * chill_height;
            vertex[j++] = 0.6 * sin(2.0 * M_PI * (float)i / STEPS);
        ...
        // отрисовываем нижнее основание
        for(int k = 0; k < STEPS; ++k){
            //first triangle
            vertex[j++] = ((float)k / STEPS) * 0.6 * cos(2.0 * M_PI * (float)i / STEPS);
            vertex[j++] = 0.0;
            vertex[j++] = ((float)k / STEPS) * 0.6 * sin(2.0 * M_PI * (float)i / STEPS);
            ...
            vertex[j++] = ((float)k / STEPS) * 0.6 * cos(2.0 * M_PI * (float)(i + 1) / STEPS);
            vertex[j++] = 0.0;
            vertex[j++] = ((float)k / STEPS) * 0.6 * sin(2.0 * M_PI * (float)(i + 1) / STEPS);
            ...
            vertex[j++] = ((float)(k + 1) / STEPS) * 0.6 * cos(2.0 * M_PI * (float)(i + 1) / STEPS);
            vertex[j++] = 0.0;
            vertex[j++] = ((float)(k + 1) / STEPS) * 0.6 * sin(2.0 * M_PI * (float)(i + 1) / STEPS);
            ...
            vertex[j++] = ((float)(k + 1) / STEPS) * 0.6 * cos(2.0 * M_PI * (float)(i) / STEPS);
            vertex[j++] = 0.0;
            vertex[j++] = ((float)(k + 1) / STEPS) * 0.6 * sin(2.0 * M_PI * (float)(i) / STEPS);
        ...
        //оторисовываем верхнее основание
        for(int k = 0; k < STEPS; ++k){
            //first triangle
            vertex[j++] = ((float)k / STEPS) * 0.6 * cos(2.0 * M_PI * (float)i / STEPS);
            vertex[j++] = chill_height;
            vertex[j++] = ((float)k / STEPS) * 0.6 * sin(2.0 * M_PI * (float)i / STEPS);
...
int main()
{
    ...
    std::cin >> SpecStren;
    std::cout << "\t4)Enter approximation parametr - num of the step: ";
    std::cin >> STEPS;
    std::cout << "\n--------------------------------" << std::endl;
    ...
    glBufferData(GL_ARRAY_BUFFER, 2 * sizeof(GLfloat) * STEPS * 3 * STEPS * 18, vertices, GL_STATIC_DRAW);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glUniform3f(objectColorLoc, 0.2f, 0.35f, 1.0f);
        ...
        GLint intime = glGetUniformLocation(lightingShader.Program,  "time");
        ...
        glDrawArrays(GL_TRIANGLES, 0, STEPS * 3 * STEPS * 2 * 3);
        glBindVertexArray(0);
